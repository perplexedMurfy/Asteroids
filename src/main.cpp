...
void manageState (double deltaTime) {
    ...
    switch (state.state) {
        case (IN_LEVEL): {
            if (!state.hasAsters) { state.state = INBETWEEN_LEVEL; state.timer = 0; }
...
/*
    ...
    There is respawn protections, ship always spawns in the center, checks if there are asters nearby so it doesn't spawn the player unfairly.
        This measure doesn't prevent all deaths...
        We should let the player tell the game when to respawn them
...
int main (int argc, char* argv[]) {
    ...
    { //checks renderer info for required features
        ...
        if (error) { printf("SDL error\n"); return -1; }
        if ((renInfo->flags & SDL_RENDERER_TARGETTEXTURE) &&
            (renInfo->max_texture_width >= REN_WIDTH &&
             renInfo->max_texture_width >= REN_HEIGHT)) {
            free (renInfo);
        }
        else {
    ...
    texPreTarget = SDL_CreateTexture(ren, SDL_PIXELFORMAT_RGBA32,
                                     SDL_TEXTUREACCESS_TARGET, REN_WIDTH, REN_HEIGHT);
    ...
    while (!quit) {
        ...
        if (deltaTime >= GAME_TICK_RATE) {
            ...
            if (state.playerEntID != -1) {
                ...
                if (keyIsPressed (SDL_SCANCODE_RIGHT)) {
                    comLocMap.at(state.playerEntID).pos.x += 0.5;
                }
                if (keyIsPressed (SDL_SCANCODE_DOWN)) {
                    comLocMap.at(state.playerEntID).pos.y += 0.5;
                }
            ...
            if (keyIsPushed (SDL_SCANCODE_X)) {
                makeUFO(UFO_SMALL);
            }
            if (keyIsPushed (SDL_SCANCODE_C)) {
                makeUFO(UFO_BIG);
            }
            ...
            if (keyIsPressed(SDL_SCANCODE_P)) {
            ...
            while (iter != entList.end()) {
                ...
                if (rendermode)
                    renderPoints(coms.poi, coms.loc);
                else
            ...
            if (!colEveList.empty()) {
                for (auto iter = colEveList.begin();
                     iter != colEveList.end();
                     iter = colEveList.erase(iter)) {
            ...
            if (!bulSpawnEveList.empty()) {
                for (auto iter = bulSpawnEveList.begin();
                     iter != bulSpawnEveList.end();
                     iter = bulSpawnEveList.erase(iter)) {
