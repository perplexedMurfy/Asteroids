...
double v2::getAngle () {
    ...
    if      ( xIsPos &&  yIsPos) { offset = RAD(0); }
    else if ( xIsPos && !yIsPos) { offset = RAD(360); }
    else if (!xIsPos &&  yIsPos) { offset = RAD(180); }
    else if (!xIsPos && !yIsPos) { offset = RAD(180); }
...
v2 v2::operator+ (const v2& rhs) const { v2 lhs(*this); return lhs += rhs; }
...
v2 v2::operator- (const v2& rhs) { v2 lhs(*this); return lhs -= rhs; }
...
/** vec dot vec */
//dot product
double v2::dot (const v2& rhs) {
    return (rhs.x * this->x) + (rhs.y * this->y);
}

/** vec cross vec */
//cross product
double v2::cross (const v2& rhs) {
...
mat2 mat2::operator+ (const mat2& rhs) const { mat2 lhs(*this); return lhs += rhs; }
...
v2 mat2::operator* (const v2& rhs) {
    return v2 (this->MAT_A * rhs.x + this->MAT_B * rhs.y,
               this->MAT_C * rhs.x + this->MAT_D * rhs.y);
}
...
mat2 mat2::operator* (const double& rhs) { mat2 lhs (*this); return lhs *= rhs; }
...
double mat2::determinant () {
    return (MAT_A * MAT_D) - (MAT_B * MAT_C);
}
 ...
 */
mat2 mat2::transpose () {
    return mat2 (MAT_A, MAT_C, MAT_B, MAT_D);
}
