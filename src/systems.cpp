...
void renderColliders(const comLocation *loc, const comCollider *col) {
	...
	for (int i = 0; i < col->len; i++) {
		...
		SDL_RenderDrawLine(ren, (int)pair.one.x, (int)pair.one.y, (int)pair.two.x, (int)pair.two.y);
	}
}
...
void advanceAnimation(comAnimation *ani, const comLocation *loc, double delta) {
	...
	while (ani->elapsedTime > ani->rate) {
		ani->curImage++;
		if (ani->curImage > ani->totalImages) { ani->curImage = 1; }
...
void moveEntity(comMovement *mov, comLocation *loc, double delta) {
	...
	// wrap arounds
	if (loc->pos.x > REN_WIDTH) { loc->pos.x -= REN_WIDTH; }
	if (loc->pos.y > REN_HEIGHT) { loc->pos.y -= REN_HEIGHT; }
	if (loc->pos.x < 0) { loc->pos.x += REN_WIDTH; }
	if (loc->pos.y < 0) { loc->pos.y += REN_HEIGHT; }
}
...
bool checkColliderBB(v2Pair a, v2Pair b) {
	...
	// preform check
	if (((a.two.x > b.one.x) && (a.two.y > b.one.y)) && ((a.one.x < b.two.x) && (a.one.y < b.two.y))) { return true; }
	return false;
}
			...
			if (otherCol.grp != col->grp) {
				for (int i = 0; i < col->len; i++) {
					for (int j = 0; j < otherCol.len; j++) {
						...
						v2Pair b = {(rotation * otherCol.ptPairs[j].one) + otherLoc.pos,
						            (rotation * otherCol.ptPairs[j].two) + otherLoc.pos};
						if (checkColliderBB(a, b)) {
							// matrix math time
							mat2 foo(a.two.x - a.one.x, b.one.x - b.two.x, a.two.y - a.one.y, b.one.y - b.two.y);
...
void generateColliders(const comCollider *col, const comMovement *mov, const double delta) {
	if (!col || !mov || !col->depsOnMov) { return; }
...
bool advanceLifetime(comLifetime *lif, const double delta) {
	...
	lif->elapsed += delta;
	if (lif->elapsed > lif->length) { return true; }
	return false;
}
 ...
 */
bool doUFOai(const int entID, comUFO *ufo, comMovement *mov, comLocation *loc, const double delta,
             const comLocation *playerLoc) {
	...
	if (ufo->direction == UFO_LEFT) {
		if (ufo->lastPos.x < loc->pos.x) { return true; }
	} else {
		if (ufo->lastPos.x > loc->pos.x) { return true; }
	}
