...
void delLocationCom(int entID) { comLocMap.erase(entID); }
/////COM LOCATION END
...
void delMovementCom(int entID) { comMovMap.erase(entID); }
/////COM MOVEMENT END
...
void delRectangleCom(int entID) { comRecMap.erase(entID); }
/////COM RECTANGLE END
...
void delAsteroidCom(int entID) { comAstMap.erase(entID); }
/////COM ASTEROID END
...
void delLifetimeCom(int entID) { comLifMap.erase(entID); }
/////COM LIFETIME END
...
void delShooterCom(int entID) { comShrMap.erase(entID); }
/////COM SHOOTER END
...
void delAnimationCom(int entID) {
	try {
		...
		for (int i = 0; i < com.totalImages; i++) { free(com.images[i].pts); }
		free(com.images);
