...
void addCollisionEvent (int entID, int causeEntID) {
    ...
    if (causeComs.sht) { event.cause = BULLET; event.bulletOwner = causeComs.sht->owner; }
...
//TODO: for some reason there is a bogus collision even that happens when the player dies? This could cause werid things to happen if we don't fix it, even if the chances of it are low.
//after some testing, I'm not sure if this actually happens?
void processCollisionEvent (CollisionEvent event) {
    if (event.entID == state.playerEntID) {
        ...
        state.playerEntID = -1;
        if (state.lives == 0) { state.state = GAME_OVER; }
        else                  { state.state = RESPAWN_COOLDOWN; }
        state.timer = 0;
    }
...
void processBulletSpawnEvent (BulletSpawnEvent event) {
    makeBullet (event);
}