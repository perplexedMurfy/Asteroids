...
int makeBullet (BulletSpawnEvent e) {
    return makeBullet (e.pos, e.vel, e.group, e.owner);
}
...
void makeDebris (comLocation *loc, comPoints *poi, double radius, int amount) {
    ...
    if (poi) {
        for (int i = 0; i < poi->len - 1; i++) {
            makeDebrisLine(loc->pos, loc->rot, poi->pts[i], poi->pts[i+1]);
        }
    }
}
